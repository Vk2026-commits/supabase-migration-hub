...
export function OfficerEmployeeOnboarding({ userId, officerId, onEnsureProfile, onChanged }: Props) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        const nextI9 = pdfUrl(i9Bytes); const nextW4 = pdfUrl(w4Bytes);
        setI9Url(previous => { if (previous.startsWith("blob:")) URL.revokeObjectURL(previous); return nextI9; });
        setW4Url(previous => { if (previous.startsWith("blob:")) URL.revokeObjectURL(previous); return nextW4; });
      } catch (error) { console.error("Official form preview failed", error); }
    }, 500);
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    if (!/^\d{3}-\d{2}-\d{4}$/.test(ssn)) { toast.error("Re-enter your SSN on the I-9 step so the official I-9 and W-4 can be securely generated"); setCurrentStep(1); return; }
    setSubmitting(true);
    try {
      await saveSensitiveForStep(currentStep);
      const [i9Bytes, w4Bytes] = await Promise.all([buildI9(data, ssn), buildW4(data, ssn)]);
      const basePath = `${userId}/${packetIdRef.current}`;
      const i9Path = `${basePath}/form-i9.pdf`; const w4Path = `${basePath}/form-w4.pdf`;
      const [i9Upload, w4Upload] = await Promise.all([
        supabase.storage.from("onboarding-documents").upload(i9Path, new Blob([i9Bytes as unknown as BlobPart], { type: "application/pdf" }), { upsert: true, contentType: "application/pdf" }),
        supabase.storage.from("onboarding-documents").upload(w4Path, new Blob([w4Bytes as unknown as BlobPart], { type: "application/pdf" }), { upsert: true, contentType: "application/pdf" }),
      ]);
      if (i9Upload.error) throw i9Upload.error;
      if (w4Upload.error) throw w4Upload.error;
      const { error } = await (supabase as any).from("officer_onboarding_packets").update({ status: "submitted", current_step: 7, form_data: data, signature_name: data.signatureName, signature_date: data.signatureDate, i9_document_path: i9Path, w4_document_path: w4Path, submitted_at: new Date().toISOString(), updated_at: new Date().toISOString() }).eq("id", packetIdRef.current);
