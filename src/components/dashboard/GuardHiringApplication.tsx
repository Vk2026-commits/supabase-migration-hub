...
export function GuardHiringApplication({ userId, officerId, onChanged, onEnsureProfile }: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (officerId) requests.push(supabase.from("work_history").select("*").eq("officer_id", officerId).order("start_date", { ascending: false }), supabase.from("job_applications").select("id,job_posting:job_postings(title,location,company:company_profiles(company_name))").eq("officer_id", officerId));
