...
const HireButton = ({ officerId, officerName, companyId, hiringApplicationId, jobApplicationId, jobTitle, onChanged }: HireButtonProps) => {
  ...
  return <Dialog open={open} onOpenChange={prepareDialog}>
    <DialogTrigger asChild><Button className="w-full bg-indigo-600 text-white shadow-sm hover:bg-indigo-700 sm:w-auto" disabled={offerIsAccepted}>{offerCanBeRevised ? <RefreshCw className="mr-2 h-4 w-4" /> : <Briefcase className="mr-2 h-4 w-4" />}{offerIsAccepted ? "Offer Accepted" : offerCanBeRevised ? "Revise & Resend Offer" : "Send Offer"}</Button></DialogTrigger>
