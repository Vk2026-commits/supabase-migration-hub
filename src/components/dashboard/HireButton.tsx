...
const HireButton = ({ officerId, officerName, companyId, hiringApplicationId, jobApplicationId, jobTitle, onChanged }: HireButtonProps) => {
  ...
  return <Dialog open={open} onOpenChange={prepareDialog}>
    <DialogTrigger asChild><Button className="h-9 w-full min-w-0 bg-indigo-600 px-3 text-xs text-white shadow-sm hover:bg-indigo-700" disabled={offerIsAccepted}>{offerCanBeRevised ? <RefreshCw className="mr-1.5 h-3.5 w-3.5 shrink-0" /> : <Briefcase className="mr-1.5 h-3.5 w-3.5 shrink-0" />}<span className="truncate">{offerIsAccepted ? "Offer accepted" : offerCanBeRevised ? "Revise offer" : "Send offer"}</span></Button></DialogTrigger>
